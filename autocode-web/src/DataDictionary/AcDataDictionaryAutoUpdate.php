...
class AcDataDictionaryAutoUpdate{
    ...
    {
        $apiUrl = $acDataDictionaryAutoApi->urlPrefix . '/' . $acDDTable->tableName . "/" . $acDataDictionaryAutoApi->pathForUpdate;        
    ...
    private function getAcApiDocRoute(): AcApiDocRoute{
        ...
        $acApiDocRoute->summary = "Update " . $this->acDDTable->tableName;
        $acApiDocRoute->description = "Auto generated data dictionary api to update row in table " . $this->acDDTable->tableName.". Either single row or multiple rows can be updated at a time.";
        ...
        $responses = AcApiDocUtils::getApiDocRouteResponsesForOperation(operation:AcEnumDDRowOperation::UPDATE, acDDTable:$this->acDDTable,acApiDoc:$this->acDataDictionaryAutoApi->acWeb->acApiDoc);
    ...
    private function getHandler(): callable{
        $handler = function (AcWebRequest $acWebRequest): AcWebResponse {
            ...
            if(isset($acWebRequest->body['row'])){
                $response = $acSqlDbTable->updateRow(row: $acWebRequest->body['row']);
            }
            else if(isset($acWebRequest->body['rows'])){
                $response = $acSqlDbTable->updateRows(rows: $acWebRequest->body['rows']);
            }
            else{
