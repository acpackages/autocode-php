...
class AcDataDictionaryAutoInsert
{
    ...
    {
        $apiUrl = $acDataDictionaryAutoApi->urlPrefix . '/' . $acDDTable->tableName . "/" . $acDataDictionaryAutoApi->pathForInsert;
    ...
    private function getAcApiDocRoute(): AcApiDocRoute
    {
        ...
        $acApiDocRoute->summary = "Insert " . $this->acDDTable->tableName;
        $acApiDocRoute->description = "Auto generated data dictionary api to insert row in table " . $this->acDDTable->tableName.". Either single row or multiple rows can be added at a time.";
    ...
    private function getHandler(): callable
    {
        $handler = function (AcWebRequest $acWebRequest): AcWebResponse {
            ...
            if(isset($acWebRequest->body['row'])){
                $response = $acSqlDbTable->insertRow(row: $acWebRequest->body['row']);
            }
            else if(isset($acWebRequest->body['rows'])){
                $response = $acSqlDbTable->insertRows(rows: $acWebRequest->body['rows']);
            }
            else{
